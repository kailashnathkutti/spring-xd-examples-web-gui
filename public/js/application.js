$(function() {
	...
	function positionSuccess(position) {
		...
		// load leaflet map
		map = L.map('map');
		map.setView([1.364556,103.817195], 12);
		...
		L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 18, detectRetina: true }).addTo(map);
	...
	function setMarker(data4) {
	
		var arr = eval( data4 );
		for (var i = 0; i < arr.length; i++) {
			var marker = L.marker([arr[i][0], arr[i][1]], { icon: yellowIcon }).addTo(map);
			marker.bindPopup('<p>New customer</p>');
			markers[data4.id] = marker;
		}
	}
